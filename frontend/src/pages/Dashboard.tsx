  ...
  return (
    <div className="space-y-6 w-full max-w-full flex-1 flex flex-col justify-between">
        ...
        <div className="xl:col-span-4 relative rounded-3xl p-6 overflow-hidden bg-gradient-to-b from-slate-900 via-slate-900/90 to-slate-950 border border-slate-800 shadow-2xl flex flex-col justify-between">
          ...
          <div className="grid grid-cols-3 gap-2.5 mt-4 pt-4 border-t border-slate-800/80">
            ...
            <div className="glass-panel p-3 rounded-2xl border border-slate-800 text-center space-y-0.5">
              ...
              <span className="text-xl font-black text-emerald-400">{completedJobsCount}</span>
            </div>
            <div className="glass-panel p-3 rounded-2xl border border-slate-800 text-center space-y-0.5">
              <span className="text-[10px] font-semibold uppercase text-slate-400 block">Active</span>
              <span className="text-xl font-black text-indigo-400">{processingJobsCount}</span>
            </div>
          </div>
        </div>
        ...
        <div className="xl:col-span-8 glass-card p-6 rounded-3xl relative overflow-hidden shadow-2xl border border-slate-800/80 flex flex-col justify-between">
          <div className="flex items-center justify-between border-b border-slate-800/80 pb-3 mb-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-base font-bold text-white tracking-tight">Upload Product Asset</h2>
                <p className="text-[11px] text-slate-400">Input product metadata and optional image asset</p>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 flex-1 flex flex-col justify-between">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl bg-slate-950/80 border border-slate-800 text-slate-100 placeholder-slate-500 text-xs focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Key features, target audience, color accents, or style preferences..."
                    className="w-full px-3.5 py-2.5 rounded-xl bg-slate-950/80 border border-slate-800 text-slate-100 placeholder-slate-500 text-xs focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all resize-none"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                {previewUrl ? (
                  <div className="relative h-[135px] rounded-xl overflow-hidden border border-indigo-500/30 bg-slate-950 p-3 flex items-center justify-between shadow-inner">
                    <div className="flex items-center space-x-3">
                      <img
                        src={previewUrl}
                        alt="Uploaded Preview"
                        className="w-20 h-20 object-cover rounded-lg border border-slate-800 shadow-md"
                      />
                      <div className="truncate space-y-1">
                        <p className="text-xs font-bold text-slate-100 truncate max-w-[180px]">{selectedFile?.name}</p>
                        <span className="inline-block px-2 py-0.5 rounded-md bg-indigo-500/10 text-indigo-300 text-[10px] font-mono">
                          {(selectedFile!.size / 1024 / 1024).toFixed(2)} MB
                        </span>
                        <p className="text-[10px] text-emerald-400 font-medium">Ready for AI Vision Analysis</p>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className="px-2.5 py-1.5 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-400 text-xs font-semibold flex items-center space-x-1 transition-colors border border-red-500/20"
                    >
                  ...
                  <label className="flex flex-col items-center justify-center h-[135px] border-2 border-dashed border-slate-800 hover:border-indigo-500/50 rounded-xl bg-slate-950/40 hover:bg-slate-950/80 cursor-pointer transition-all duration-200 group p-3">
                    <div className="p-2 rounded-xl bg-slate-900 group-hover:bg-indigo-500/10 text-slate-400 group-hover:text-indigo-400 transition-colors mb-1.5 border border-slate-800">
                      <Upload className="w-4 h-4" />
                    </div>
                    <span className="text-xs font-bold text-slate-300">Click or Drag & Drop Product Image</span>
                    <span className="text-[10px] text-slate-500 mt-0.5">High resolution PNG, JPG, or WebP up to 10MB</span>
            ...
            <button
              ...
              className="w-full py-3 px-5 rounded-xl bg-gradient-to-r from-indigo-600 via-purple-600 to-sky-600 hover:from-indigo-500 hover:to-sky-500 text-white font-extrabold text-xs transition-all duration-300 shadow-lg shadow-indigo-600/20 flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed group mt-2"
            >
      ...
      {activeJobId && activeJob && (
        <section className="glass-panel p-6 rounded-3xl border border-indigo-500/30 bg-slate-900/90 space-y-5 shadow-2xl relative overflow-hidden w-full">
          ...
          {activeJob.status === 'Completed' && (
            <div className="p-3.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-between text-emerald-400 animate-fadeIn shadow-lg">
          ...
          {activeJob.status === 'Completed' && activeJob.generated_image_url && (
            <div className="space-y-5 pt-2 animate-fadeIn">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 p-3.5 rounded-2xl bg-slate-950 border border-slate-800">
                <div className="flex items-center space-x-2">
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs flex items-center space-x-2 transition-colors shadow-md"
                  >
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="px-3 py-1.5 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 hover:text-white font-semibold text-xs flex items-center space-x-1.5 transition-colors"
                  >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 p-3.5 rounded-2xl bg-slate-950 border border-slate-800 text-xs">
              ...
              {activeJob.generated_prompt && (
                <div className="rounded-2xl bg-slate-950 border border-slate-800 overflow-hidden">
                  <button
                    ...
                    className="w-full p-3.5 flex items-center justify-between text-xs font-bold text-slate-300 hover:text-white transition-colors bg-slate-900/60"
                  >
                    <div className="flex items-center space-x-2">
                      <Sparkles className="w-4 h-4 text-indigo-400" />
                  ...
                  {isPromptExpanded && (
                    <div className="p-4 space-y-3 border-t border-slate-800/80 animate-fadeIn">
                      ...
                      <pre className="text-xs font-mono bg-slate-900 p-3.5 rounded-xl text-indigo-300 border border-slate-800/80 max-h-56 overflow-y-auto whitespace-pre-wrap leading-relaxed">
                        {activeJob.generated_prompt}
                      </pre>
                    </div>
                  )}
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-8 pr-3 py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-200 placeholder-slate-500 text-xs focus:outline-none focus:border-indigo-500 transition-all w-40 sm:w-52"
              />
            </div>
            ...
            <button
              ...
              className="p-1.5 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-400 hover:text-slate-200 text-xs font-medium transition-all"
              title="Refresh Jobs"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
            {filteredJobs.map((job) => (
              <div
                ...
                className={`glass-card glass-card-hover p-4 rounded-2xl flex flex-col justify-between space-y-3 cursor-pointer relative group ${
              ...
              >
                <div className="space-y-2.5">
                  ...
                  <div className="w-full h-36 rounded-xl overflow-hidden bg-slate-950 border border-slate-800/80 flex items-center justify-center relative shadow-inner">
                ...
                <div className="pt-2.5 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
                  ...
                  <div className="flex items-center space-x-1 text-indigo-400 group-hover:translate-x-1 transition-transform font-bold bg-indigo-500/10 px-2 py-0.5 rounded-lg border border-indigo-500/20 text-[10px]">
                    <span>View</span>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="glass-panel w-full max-w-3xl max-h-[85vh] rounded-3xl overflow-hidden flex flex-col border border-slate-700 shadow-2xl animate-fadeIn">
            <div className="p-5 border-b border-slate-800 flex items-center justify-between bg-slate-900/60">
              <div>
                <div className="flex items-center space-x-3">
                  <h3 className="text-sm font-bold text-white">Job #{selectedJob.id} Specifications</h3>
                  {getStatusBadge(selectedJob.status)}
                </div>
                <p className="text-[11px] text-slate-400 mt-0.5">Submitted on {new Date(selectedJob.created_at).toLocaleString()}</p>
              </div>
              <button
            ...
            <div className="p-5 overflow-y-auto space-y-5">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-white">{selectedJob.product_name}</p>
                {selectedJob.product_description && (
                  <p className="text-xs text-slate-300 mt-1.5 bg-slate-950 p-3 rounded-xl border border-slate-800 leading-relaxed">
                    {selectedJob.product_description}
                  </p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {selectedJob.uploaded_image_path && (
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block mb-1.5">Original Uploaded Product</span>
                    <img
                      ...
                      className="w-full h-44 object-cover rounded-xl border border-slate-800 cursor-zoom-in"
                    />
                  </div>
                )}
                ...
                {selectedJob.generated_image_url && (
                  <div>
                    ...
                    <img
                      ...
                      className="w-full h-44 object-cover rounded-xl border border-indigo-500/40 cursor-zoom-in"
                    />
                  </div>
                )}
              </div>
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className="px-3.5 py-1.5 rounded-xl bg-indigo-600/20 hover:bg-indigo-600/30 border border-indigo-500/40 text-indigo-300 font-bold text-xs flex items-center space-x-1.5 transition-colors"
                >
                  <Download className="w-3.5 h-3.5" />
              ...
              {selectedJob.generated_prompt && (
                <div>
                  ...
                  <pre className="text-xs font-mono bg-slate-950 p-3.5 rounded-2xl text-indigo-300 border border-slate-800 overflow-x-auto whitespace-pre-wrap leading-relaxed">
                    {selectedJob.generated_prompt}
                  </pre>
                </div>
              )}
